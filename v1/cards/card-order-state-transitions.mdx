...
<Warning>
**State Validation**: Always validate the current order state before attempting transitions. Invalid transitions will throw a `TransitionError` and return a HTTP 422 status code.
</Warning>
