...
When you sign up for Gnosis Pay, a new Gnosis Pay Safe—a smart contract wallet—is created for you.This Safe holds your funds on-chain and is controlled only by its **owners**.Gnosis Pay Safe owner can be either an EOA wallet (Externally Owned Account) , or another smart wallet (e.g. a Safe) whose owners may use passkeys, EOAs, or other signers.
...
## Authenticating user sessions with wallets

When you log in to the Gnosis Pay dashboard or call our APIs, you use a wallet to complete [Sign-In With Ethereum (SIWE)](https://docs.login.xyz/). This wallet authenticates your identity and issues a JWT token after SIWE. You can read more about this in our [Authentication Flow](https://docs.gnosispay.com/auth).

With this token, you can interact with the Gnosis Pay APIs — e.g., ordering cards, fetching user info, or adding more wallets for authenticating your user sessions.

This wallet address does not perform onchain transactions. The authenticated wallet may or may not be the same as the wallet that owns your Gnosis Pay Safe.

Follow this guide to [learn how to add and authenticate](account/update-authenticated-account) different wallets for your Gnosis Pay sessions.
<Tip>
    The first wallet that is connected to your session automatically becomes your primary owner for your Delay Module that is owned by Gnosis Pay Safe Account.
</Tip>
