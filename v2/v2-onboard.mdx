...
Register a new user with email OTP and create a user profile. This endpoint will return a new access token with userID that will be used as bearer token for all the subsequent API requests. Optionally provide a shareToken for Sumsub reusable KYC.
...
<Step title="Accept Terms of Service (ToS)">
    ...
    You can check the status of the user's acceptance of the Terms of Service via the [`GET /user/terms`](/api-reference/terms/get-user-terms) endpoint.
...
<Steps>
    ...
    <Step title="Check onboarding status">
        Check the user's onboarding status using the `GET /user/onboarding`endpoint to determine if their KYC verification has been accepted. If the KYC is approved, the user status will transition to `action_create_account`. In next step, we will create GP spending safe wallet address.
    </Step>
    ...
    <Step title="Verify Phone Number">
        ...
        Call the `GET /user/onboarding` endpoint to check the status.
        ...
        **Submit Phone Number**: Submit the phone number for verification purposes and to enable card creation.
        ...
        **Submit OTP**: After submitting the phone number, a One-Time Password (OTP) will be received via SMS. Submit this OTP to complete the phone verification process.
        ...
        Once the OTP is successfully verified, call `GET /user/onboarding`. At this stage, the user can create virtual cards and the user onboarding is fully completed.
        ```json
        {
            "status": "completed"
        }
