...
The Roles module supports multi-token configurations so that users can spend different currencies via their spending safe.
...
The Roles module enforces a hardcoded on-chain limit of **\$20K per token***, twice the card limit. This buffer exists because card clearing and settlement can take 1–4 days after a transaction, and the higher onchain limit ensures uninterrupted card operations over that window without requiring any additional user signatures.
