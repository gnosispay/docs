...
Once the OTP verification is successful (previous step), and the user has reviewed and accepted the ToS, you can call the endpoint to accept each ToS that the user hadn't accepted yet [(spec)](/api-reference/user/get-terms-and-conditions-status):
...
Each user gets a unique URL for their KYC process. Get it with [(spec)](/api-reference/kyc/retrieves-kyc-integration-specification):
