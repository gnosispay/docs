...
To instantiate their SDK you need an access token that you can get from [(spec)](/api-reference/kyc/retrieves-kyc-integration-specification-for-sdk): 
...
```bash cURL
curl -X POST /api/v1/safe/set-currency
```
...
```bash cURL
curl -X GET /api/v1/account/signature-payload
```
...
```bash cURL
curl -X PATCH /api/v1/account/deploy-safe-modules \
-d '{
