---
title: Onboard Users to Gnosis Pay
description: How to onboard new Users to Gnosis Pay.
---
...
Here's the endpoint [(spec)](/api-reference/authentication/request-otp-for-email-verification):
...
Here's the endpoint [(spec)](/api-reference/authentication/create-a-new-user):
```jsx
...
Get the terms for the user [(spec)](/api-reference/user/get-terms-and-conditions-status):
```jsx
...
To order a card, your users will need to go through a KYC process with our partner Sumsub. The whole KYC flow happens in a Sumsub iframe, and in the background, the Gnosis Pay API keeps you updated on your user's `kycStatus` (check it with [`GET /api/v1/user`](/api-reference/user/get-the-user-profile)).
...
Get it with [(spec)](/api-reference/kyc/retrieves-kyc-integration-specification):

```jsx
GET /api/v1/kyc/integration
```

For mobile integrations, you can use the Sumsub Mobile SDK. To instantiate their SDK you need an access token that you can get from [(spec)](/api-reference/kyc/retrieves-kyc-integration-specification-for-sdk): 
```jsx
...
You can get the list of questions and possible answers at this endpoint [(spec)](/api-reference/kyc/retrieve-the-source-of-funds-questions): 

```jsx
GET /api/v1/source-of-funds
```

Show these questions to your user and collect their answers. You'll need to send all the answers together in one go [(spec)](/api-reference/kyc/submit-answers-to-the-source-of-funds-questionnaire):
...
Validate the phone number by first requesting an OTP [(spec)](/api-reference/kyc/request-an-otp-to-verify-a-phone-number): 
...
This will send a code to the user's phone number, which in turn needs to be passed on to the check endpoint [(spec)](/api-reference/kyc/validates-an-otp-code-to-verify-a-phone-number): 
...
1. Creating a Safe account [(`POST /api/v1/account`)](/api-reference/safe-management/create-or-deploy-a-safe-account-for-the-authenticated-user)
2. Setting the Safe currency [(`POST /api/v1/safe/set-currency`)](/api-reference/safe-management/set-the-currency-for-a-users-safe-account-based-on-their-country)
3. Getting signature data for modules setup [(`GET /api/v1/account/signature-payload`)](/api-reference/safe-management/get-signature-data-for-account-setup)
4. Deploying Safe modules with the user's signature [(`PATCH /api/v1/account/deploy-safe-modules`)](/api-reference/safe-management/deploy-safe-modules-with-a-user-signature)
...
<Steps titleSize="h3">
<Step title="Create a Safe Account">
The first step is to create and deploy a Safe account for the user. [(spec)](/api-reference/safe-management/create-or-deploy-a-safe-account-for-the-authenticated-user)
```tsx
POST /api/v1/account
{
...
<Note>
After this, `safeWallet` from the user response from  [`GET /api/v1/user` endpoint](/api-reference/user/get-the-user-profile) won't be empty anymore.
</Note>
</Step>

<Step title="Set the Safe Currency">
...
<Note>
Before doing this, note that the fields `tokenSymbol` and `fiatSymbol` are `null` in the response from [`GET /api/v1/safe-config`](/api-reference/safe-management/retrieve-the-safe-configuration-for-the-authenticated-user).
</Note>

Use the following endpoint to set the currency [(spec)](/api-reference/safe-management/set-the-currency-for-a-users-safe-account-based-on-their-country):
...
<Step title="Getting Signature Data for Modules Setup">
...
<Note>
Before this, note that the field `accountStatus` is `null` in the response from [`GET /api/v1/safe-config`](/api-reference/safe-management/retrieve-the-safe-configuration-for-the-authenticated-user).
</Note>

You can get the data to be signed with the following endpoint  [(spec)](/api-reference/safe-management/get-signature-data-for-account-setup).
```tsx
...
<Step title="Signing and Deploying Safe Modules">

The final step is to sign the data with the user's wallet and deploy the Safe modules.

<Note>
Before this, note that the field `accountStatus` is a number that is not `0` in the response from [`GET /api/v1/safe-config`](/api-reference/safe-management/retrieve-the-safe-configuration-for-the-authenticated-user). `accountStatus: 0` would mean that the Safe modules are already deployed.
</Note>

You can use the library [`@gnosispay/account-kit`](https://github.com/gnosispay/account-kit) to have a handy `AccountIntegrityStatus` enum.
...
Send the signature to deploy the Safe modules [(spec)](/api-reference/safe-management/deploy-safe-modules-with-a-user-signature):
