...
<Warning>
  **Important**: The same event may be delivered multiple times. Always design
  your webhook handlers to process events idempotently.
</Warning>
...
async function processWebhookEvent(eventType: string, data: any) {
  try {
    ...
    // Don't throw - return 2xx to acknowledge receipt
...
async function handleUserCreated(data: unknown) {
  ...
  const validatedData = UserCreatedSchema.parse(data);
