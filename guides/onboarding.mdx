...
```
  ...
  <Step title="Get onboarding status of user">
    ...
    | `action_verify_phone` | Phone verification required - the account is active, but the user can't create a card until the phone is verified. You only see this status if you required phone OTP for your users | Verify the phone number with OTP. See [Phone Number](/guides/phone) |
    | `completed` | Onboarding complete - account is active | Start using the platform |
...
<Steps>
  <Step title="Handle phone verification requirement">
    Once the account has provisioned and is active, [`GET /user/onboarding`](/api-reference/user/get-onboarding-status) returns `action_verify_phone` if you required phone OTP for your users.
...
```
    Prompt the user to verify their phone number to proceed: send a code with [`POST /phone/verification`](/api-reference/phone/request-phone-verification), then confirm it with [`POST /phone`](/api-reference/phone/set-phone-number). Once verification is complete, the onboarding status will move to `completed`. See [Phone Number](/guides/phone) for the full flow.

    If you did not require phone OTP for your users, the status goes straight to `completed` and the phone number is set when the first card is created.
  </Step>
</Steps>
