...
The request accepts an optional `phone` field (E.164 format). The behavior depends on whether the user already has a phone number, and on whether you require phone OTP for your users. See [Phone Number](/guides/phone).

| Scenario | Result |
|----------|--------|
| User already has a phone number | Card created directly; `phone` is ignored |
| You require phone OTP for your users, and the user has no phone yet | `409 PHONE_VERIFICATION_REQUIRED`. Verify the number with [`POST /phone/verification`](/api-reference/phone/request-phone-verification) then [`POST /phone`](/api-reference/phone/set-phone-number) first |
| You do not require phone OTP, no phone on the user, and no `phone` in the request | `422 PHONE_REQUIRED` |
| You do not require phone OTP, no phone on the user, valid `phone` in the request | Phone is set, then the card is created |
