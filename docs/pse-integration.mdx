...
For security reasons, the only way to apply custom styling to iframe elements is to prepare and share a **CSS file** with the Gnosis Pay team. This file, named `<partner-id>.css`, will be incorporated into the iframe.
...
1.  Locate the custom CSS file with your name or ID in either the "**Style Editor**" in Firefox or the "**Sources**" panel. In the example below, the file is `gnosis_pay_ui.css`.
